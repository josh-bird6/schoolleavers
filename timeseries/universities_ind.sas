/*********************
Bringing in names from hesasr directory 2018-19
*********************/
...
proc sort data= schoolleaver_tomerge1819
out =schoolleaver_tomerge1819;
by  scno identifier;
run;
/*************
*************
...
*************
*************/
data schoolleaver_unimerged1819;
merge ay_201819_uni(in=a) schoolleaver_tomerge1819 (in=b);
by scno identifier;
if a ;
...
*************
*************/
proc sort data = schoolleaver_unimerged1819
out = schoolleaver_unimerged1819;
by ukprn_school;
run;

proc sort data = work.impw8041
out = ukprn_tomerge_1819;
by ukprn_school;

run;

data hesa_schoolleavers1819;
merge schoolleaver_unimerged1819 (in=a) ukprn_tomerge_1819(in=b);
by ukprn_school;
...
data hesa_schoolleavers_LA1819;
set hesa_schoolleavers1819;
if schoolleaver=1;
run;
/*********INDEPENDENTS201718************************/
/*********INDEPENDENTS201718************************/
...
proc sort data= ay_201718_uni
out = ay_201718_uni;
by scno identifier;
run;

data schoolleaver_tomerge1718;
set work.impw5833;
...
proc sort data= schoolleaver_tomerge1718
out =schoolleaver_tomerge1718;
by  scno identifier;
run;
/*************
*************
...
*************
*************/
data schoolleaver_unimerged1718;
merge ay_201718_uni(in=a) schoolleaver_tomerge1718 (in=b);
by scno identifier;
if a ;
if schoolleaver ne 1 then schoolleaver = 0;
run; 
...
*************
*************/
proc sort data = schoolleaver_unimerged1718
out = schoolleaver_unimerged1718;
by ukprn_school;
run;

proc sort data = work.impw8041
out = ukprn_tomerge_1718;
by ukprn_school;

run;

data hesa_schoolleavers1718;
merge schoolleaver_unimerged1718 (in=a) ukprn_tomerge_1819(in=b);
by ukprn_school;
...
/*********************88
FILTERING FOR LA SCHOOL LEAVERS*/
data hesa_schoolleavers_LA1718;
set hesa_schoolleavers1718;
if schoolleaver=1;
run;

/*********INDEPENDENTS201819************************/
/*********RUNNING THE FINAL BIT
************************
************************
************************
************************
...
************************
************************/
data hesa_schoolleavers_ind1819;
set hesa_schoolleavers1819;
...
************************
************************/
data hesa_schoolleavers_ind1718;
set hesa_schoolleavers1718;

if schoolleaver = 0;
if dateob >='01OCT1998'd;
if xdomhm01 =2;
